...
###################################### Sort Jobs by priority (priority scheduling)
def Priority(p):
    return p['prio']
...
#################################### Sort Jobs by first-come-first-serve basis
def JobValue(p):
    return p['job']
Jobs.sort(key=JobValue)
...
#################################### Sort Jobs by time to completion (shortest job first)
def Time(p):
    return p['time']
Jobs.sort(key=Time)
